...
pub(crate) fn write_fold_changes(
    ...
    wtr_dist
        .serialize(("feature_id", "foldchange", "probability"))
        .unwrap();
    ...
    feature_ids
        .iter()
        .try_for_each(|(_i, feature_id)| -> Result<()> {
...
pub(crate) fn write_kallisto_fold_changes(
    ...
    wtr.serialize(("feature_id", "foldchange", "probability"))
        .unwrap();
...
pub(crate) fn write_kallisto_counts(
    ...
    let mut wtr = csv::Writer::from_path(output)?;
    wtr.serialize(("sample_id", "feature_id", "normalized_count", "dispersion"))
        .unwrap();
    let feature_ids: Vec<_> = preprocessing.feature_ids().iter().enumerate().collect();
    ...
    for sample_id in sample_ids {
        ...
        feature_ids
            ...
            .try_for_each(|(i, feature_id)| -> Result<()> {
                ...
                wtr.serialize((sample_id.clone(), feature_id, mean, dispersion))
                    .unwrap();
