...
pub(crate) fn group_expression(
    ...
    let db_path = out_dir_path.to_str().unwrap().to_string();
     ...
     // --- Channel setup ---
    let (tx, rx): (Sender<(String, Vec<(f64, f64, f64)>)>, Receiver<(String, Vec<(f64, f64, f64)>)>) = channel();
    ...
    feature_ids.par_iter().try_for_each(|(i, feature_id)| -> Result<()> {
        ...
        let sample_expression_likelihoods: Vec<_> = sample_expression_paths
            .iter()
            .map(|path| ProbDistribution2d::with_readonly_connection(path.to_str().unwrap(), feature_id))
